...
void XyzWindow::CreateToolBars()
{
	...
	connect(m_XSlider , SIGNAL(valueChanged(int)) , this , SLOT(XSliderChanged(int)));
	connect(m_YSlider , SIGNAL(valueChanged(int)) , this , SLOT(YSliderChanged(int)));
	connect(m_ZSlider , SIGNAL(valueChanged(int)) , this , SLOT(ZSliderChanged(int)));
}
...
void XyzWindow::DrawXYImage()
{
	...
	//    glRasterPos2f(-1, -1);
	glRasterPos2f(offsetx , offsety);
	glDrawPixels(nx, ny, GL_LUMINANCE, GL_UNSIGNED_BYTE, sdata);
}
